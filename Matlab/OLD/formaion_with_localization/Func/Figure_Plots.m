...
plot(A(1).pos(1) + F.distance_desired(1)*sin(equi_circle), A(1).pos(2) + F.distance_desired(1)*cos(equi_circle), 'y--', 'LineWidth', 2); hold on; % equilibrium circle
plot(A(1).pos(1) + F.distance_desired(2)*sin(equi_circle), A(1).pos(2) + F.distance_desired(2)*cos(equi_circle), 'g--', 'LineWidth', 2); hold on; % equilibrium circle
plot(A(2).pos(1) + F.distance_desired(3)*sin(equi_circle), A(2).pos(2) + F.distance_desired(3)*cos(equi_circle), 'm', 'LineWidth', 2); hold on; % equilibrium circle
plot(L.x_hat(1) + F.distance_desired(3)*sin(equi_circle), L.x_hat(2) + zs_estimated*cos(equi_circle), 'c--', 'LineWidth', 2); hold on; % equilibrium circle
plot(L.x_hat(1), L.x_hat(2), 'co', 'LineWidth', 2); hold on; % estimated follower1 pos
...
% subplot(2,1,2)
% for i=1:n_agent
%     plot(t.time, BUF(i).feedback_linearized(2,:), 'LineWidth', 2); hold on;
...
% 
% for i=1:n_agent
%     plot(t.time, BUF(i).pos_dot(1,:), 'LineWidth', 2); hold on;
%     grid on; xlabel('[sec]'); ylabel('[m/s]'); 
%     title('Feedback linearization input v'); 
% end
% legend('1', '2', '3');
...
% subplot(2,1,2)
% for i=1:n_agent
%     plot(t.time, BUF(i).att(1,:), 'LineWidth', 2); hold on;
%     grid on; xlabel('[sec]'); ylabel('[rad/s]');
%     title('w'); legend(int2str(i));
% end
% legend('1', '2', '3');
...
figure,
subplot(2,1,1)
plot(t.time, BUF(3).mode(1,:), 'b.', 'LineWidth', 2);
grid on; xlabel('[sec]'); ylabel('[-]'); title('1: formation, 2: localization, 3: search leader');
ylim([1 3]);
subplot(2,1,2)
plot(t.time, BUF(3).submode(1,:), 'b.', 'LineWidth', 2);
grid on; xlabel('[sec]'); ylabel('[-]'); title('1: leader, 2: follower1, 3: follower2');
ylim([1 3]);
