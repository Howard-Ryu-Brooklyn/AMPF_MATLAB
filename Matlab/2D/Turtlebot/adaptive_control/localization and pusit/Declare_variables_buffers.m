...
v=0; w=0;
tmp = x_pos-y_pos;
y_att = atan2(tmp(2), tmp(1));

% buffer
buf_D = zeros(1, t.length);

buf_cmd = zeros(dim, t.length);
buf_cmdvw = zeros(dim, t.length);

buf_y_pos = zeros(dim, t.length);
buf_y_att = zeros(1, t.length);
buf_sigma = zeros(dim, t.length);
buf_sigma_dot = zeros(dim, t.length);
...
buf_x_hat_dot = zeros(dim, t.length);
buf_x_hat = zeros(dim, t.length);
buf_cmd_part1 = zeros(dim,t.length);
buf_cmd_part2 = zeros(dim,t.length);
